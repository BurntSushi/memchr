...
pub struct One(generic::One<v16u8>);

impl One {
    /// Create a new searcher that finds occurrences of the needle byte given.
    ///
    /// This particular searcher is specialized to use lsx vector instructions
    ...
    pub fn new(needle: u8) -> Option<One> {
        if One::is_available() {
            // SAFETY: we check that lsx is available above.
    ...
    /// Create a new finder specific to lsx vectors and routines without
    /// checking that lsx is available.
    ///
    /// # Safety
    ///
    /// Callers must guarantee that it is safe to execute `lsx` instructions
    /// in the current environment.
    ///
    /// Note that it is a common misconception that if one compiles for an
    /// `x86_64` target, then they therefore automatically have access to lsx
    ...
    /// `end` must be at least the size of a lsx vector (in bytes).
    ///
    ...
    /// Execute a count using lsx vectors and routines.
    ///
    /// # Safety
    ///
...
/// `4` and `5`.
#[derive(Clone, Copy, Debug)]
pub struct Two(generic::Two<v16u8>);
...
impl Two {
    ...
    pub unsafe fn find_raw(
        ...
        // SAFETY: Building a `Two` means it's safe to call 'lsx' routines.
        // Also, we've checked that our haystack is big enough to run on the
        // vector routine. Pointer validity is caller's responsibility.
        self.find_raw_impl(start, end)
    }
...
/// `0`, `2`, `3`, `4` and `5`.
#[derive(Clone, Copy, Debug)]
pub struct Three(generic::Three<v16u8>);
...
impl Three {
    ...
    pub unsafe fn find_raw(
        ...
        // SAFETY: Building a `Three` means it's safe to call 'lsx' routines.
        // Also, we've checked that our haystack is big enough to run on the
        // vector routine. Pointer validity is caller's responsibility.
        self.find_raw_impl(start, end)
    }
